...
StyleDictionary.registerTransform({
  ...
  transformer: (token) => {
    ...
    if (token.path[0] === 'syntax' || token.path[0] === 'semantic') {
...
StyleDictionary.registerFormat({
  ...
  formatter: (dictionary, config) => {
    ...
    dictionary.allProperties.filter((token) => {
      return !['color', 'syntax', 'semantic'].includes(token.path[0])
    }).forEach((token) => {
    ...
    for (const token of dictionary.allProperties) {
      if (token.path[0] === 'semantic') {
        theme.semanticTokenColors[token.name] = {
          foreground: token.value,
          fontStyle: token.fontStyle,
        };
      }
